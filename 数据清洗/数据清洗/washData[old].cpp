	...
	*/
	...
	for (int i = 0; i < loop; i++){
		...
		for (int j = 0; j < site; j++){
			...
			//如果最后一个点回到了开头，那么该点将被剔除.
			if (j == site - 1 && site_id == tmpSite[0])
			{
				origin = true;
				site--; //有效的site数将会减少一个
				//tmpSite.resize(site);
		...
		for (int j = 0; j < fre; j++){ // 10躺
			...
			for (int k = 0; k < site-1; k++){ //site站数
				...
				read >> start; //第一行的末
				int key;
				for (int m = 0; m < route[tmpSite[k]].size(); m++)
				{
					if (route[tmpSite[k]][m].mark == 0)
					{
						key = m;
						break;
					}
				}
				route[tmpSite[k]][key].startT = start;
				//cout << tmpSite[k] << "  " << start << endl;
				route[tmpSite[k]][key].target = tmpSite[k + 1];
				read >> arrival; //第二行的初
				//read.seekp(-1, ios_base::cur);
				route[tmpSite[k]][key].dur = arrival - start;
				//cout << route[tmpSite[k]][route[tmpSite[k]].size() - 1].dur << endl;
				route[tmpSite[k]][key].mark = MARK;
			}
			//没有计算最后一个到达点（最后一个到达点信息可以表示在入读点中）
			read >> start;
			if (origin) //存在组后一个结点无效
			{
				read >> arrival;
				read >> start;
			}
